...
router.post('/', async (req, res) => {
  ...
  if (!Number.isInteger(participantCount) || participantCount < 2 || participantCount > 5) {
    return res.status(400).json({ message: 'Team size must be between 2 and 5 (leader + at least one member).' });
  }
